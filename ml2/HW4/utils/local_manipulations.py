...
def plot_scores(model, X_tr, y_tr, X_val, y_val):
    ...
    fig, ax = plt.subplot_mosaic(mosaic, figsize=(15, 7))
    for key in ax:
        ax[key].set_title(key, fontsize=15)
        ax[key].set_yscale('log')

    sns.histplot(x=y_pred_tr_expit, hue=y_tr, bins=100, ax=ax['tr_expit'])
    sns.histplot(x=y_pred_tr_raw, hue=y_tr, bins=100, ax=ax['tr_raw'])
    sns.histplot(x=y_pred_val_expit, hue=y_val, bins=100, ax=ax['val_expit'])
    sns.histplot(x=y_pred_val_raw, hue=y_val, bins=100, ax=ax['val_raw'])

...
def knn_cross_val_score(X, y, k_list, scoring, cv=None, **kwargs):
    ...
    for train, test in cv.split(X):
        distances, indicies = clf.fit(X[train], y[train]).kneighbors(X[test], return_distance=True)
        for k in k_list:
            scores[k].append(scorer(y[test], clf._predict_precomputed(indicies[:,:k], distances[:,:k])))
